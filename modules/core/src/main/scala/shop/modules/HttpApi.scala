...
import shop.http.routes.admin.{ AdminBrandsRoutes, AdminCategoryRoutes, AdminItemsRoutes }
...
final class HttpApi[F[_]: Concurrent: Timer] private (
  ...
  private val userRoutes   = new UserSignUpRoutes[F](security.auth).routes
  ...
  // Admin routes
  private val adminBrandRoutes    = new AdminBrandsRoutes[F](algebras.brands).routes(adminMiddleware)
  private val adminCategoryRoutes = new AdminCategoryRoutes[F](algebras.categories).routes(adminMiddleware)
  private val adminItemRoutes     = new AdminItemsRoutes[F](algebras.items).routes(adminMiddleware)
